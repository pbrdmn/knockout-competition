class App {
  ...
  getTournament({ teamsPerMatch, numberOfTeams }) {
    return new Promise((resolve, reject) => {
      ...
      this.http
        .post(0, "/tournament", { teamsPerMatch, numberOfTeams })
        .then((tournament) => {
  ...
  runRound({ round, matchUps }) {
    ...
    Promise.all(
      matchUps.map((matchUp) =>
        this.getMatch({ round, match: matchUp.match }).then((matchScore) =>
          // Look up the each team by teamId for the match
          Promise.all(
            matchUp.teamIds.map((teamId) =>
              this.getTeam({ match: matchUp.match, teamId })
            )
          )
            ...
            .then(
              (teams) =>
                new Promise((resolve, reject) => {
                  ...
                  this.getWinner({ teamScores, matchScore }).then(
                    (winningScore) => {
  ...
  getMatch({ round, match }) {
    return new Promise((resolve, reject) => {
      ...
      this.http
        .get(match, "/match", { tournamentId: this.tournamentId, round, match })
  ...
  getTeam({ match, teamId }) {
    return new Promise((resolve, reject) => {
      ...
      if (this.teams[teamId]) resolve(this.teams[teamId]);
        ...
        this.http
          .get(match, "/team", { tournamentId: this.tournamentId, teamId })
