class App {
  ...
  initProgress(totalMatches) {
    if (this.ui) this.ui.initProgress(totalMatches)
  }

  getTournament({ teamsPerMatch, numberOfTeams }) {
    return this.http.post(0, '/tournament', { teamsPerMatch, numberOfTeams })
  }
  ...
  createTournament({ teamsPerMatch, numberOfTeams }) {
    return new Promise((resolve, reject) => {
      ...
      this.getTournament({ teamsPerMatch, numberOfTeams })
        .then(tournament => {
          ...
          this.initProgress(totalMatches)
    ...
    Promise.all(matchUps.map(matchUp =>
      this.getMatch({ round, match: matchUp.match })
        .then(matchScore =>
          ...
          Promise.all(matchUp.teamIds.map(teamId => this.findTeam({ match: matchUp.match, teamId })))
  ...
  getTeam(match, { tournamentId, teamId }) {
    return this.http.get(match, '/team', { tournamentId: this.tournamentId, teamId })
  }

  findTeam({ match, teamId }) {
    return new Promise((resolve, reject) => {
      ...
      if (this.teams[teamId]) resolve(this.teams[teamId])
        ...
        this.getTeam(match, { tournamentId, teamId })
