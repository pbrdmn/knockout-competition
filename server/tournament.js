...
class TournamentManager {
  ...
  createTournament(teamsPerMatch, numberOfTeams, options = {}) {
    ...
    let defaultOptions = {
      getWinningScore: (teamScores, matchScore) =>
        teamScores
          .sort((a, b) => Math.abs(a - matchScore) - Math.abs(b - matchScore))
          .shift(),
  ...
  getTournament(tournamentId) {
    ...
    if (isNaN(tournamentId)) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "Looks like you're missing a tournament ID"
      );
    }
    const tournament = this.tournaments[tournamentId];
    if (tournament == null) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.NOT_FOUND,
        "Are you sure that's the right tournament ID?"
      );
    }
    return tournament;
  }
...
class Tournament {
  ...
  static create(numberOfTeams, teamsPerMatch, options) {
    ...
    if (isNaN(teamsPerMatch)) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "Oops, you forgot to give me the teams per match"
      );
    }
    ...
    if (teamsPerMatch <= 1) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        `Can\'t run a tournament with 1 or fewer teams per match, and you tried with ${teamsPerMatch}`
      );
    }
    ...
    if (isNaN(numberOfTeams)) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "Oops, you forgot to give me the number of teams in the tournament"
      );
    }
    ...
    if (numberOfRounds == null || numberOfRounds <= 0) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "Wait... you can't make a knockout tournament with that number of teams"
      );
    }
    ...
    for (let i = 0; i < numberOfTeams; i++) {
      ...
      teams.push(
        new Team(teamId, getTeamName(i, teamId), getTeamScore(i, teamId))
      );
    }
  ...
  getWinner(teamScores, matchScore) {
    if (
      ...
      teamScores.length !== this.teamsPerMatch
    ) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "You need to give me the scores for all teams - no more, no less"
      );
    }

    teamScores = teamScores.map((teamScore) =>
      Math.floor(teamScore || undefined)
    );
    if (teamScores.some(isNaN)) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "Whoa, those scores don't look right"
      );
    }
    ...
    if (isNaN(matchScore)) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "I can't very well tell you the winner without the match score!"
      );
    }
  ...
  getTeam(teamId) {
    ...
    if (isNaN(teamId)) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "Hmm, I think you were supposed to give me a team ID"
      );
    }
    ...
    if (team == null) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.NOT_FOUND,
        "I can't find that team anywhere!"
      );
    }
  ...
  getMatch(round, match) {
    ...
    if (isNaN(round) || isNaN(match)) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.INVALID,
        "For me to give up the match data, you need to tell me the round and match numbers"
      );
    }
    ...
    if (!this.matches[round]) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.NOT_FOUND,
        "That round does not exist in this tournament"
      );
    }
    ...
    if (!this.matches[round][match]) {
      throw new TournamentError(
        TOURNAMENT_ERROR_TYPE.NOT_FOUND,
        "That match does not exist in this round"
      );
    }
